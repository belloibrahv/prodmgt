...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [totalProjects, activeProjects, completedProjects, totalTasks, openTasks, overdueTasks, teamSize] =
    await Promise.all([
      ...
      prisma.project.count({ where: { status: "ACTIVE", OR: [{ ownerId: uid }, { members: { some: { userId: uid } } }] } }),
      prisma.project.count({ where: { status: "COMPLETED", OR: [{ ownerId: uid }, { members: { some: { userId: uid } } }] } }),
      prisma.task.count({ where: { project: { OR: [{ ownerId: uid }, { members: { some: { userId: uid } } }] } } }),
      prisma.task.count({ where: { status: { notIn: ["DONE", "CANCELLED"] as never[] }, project: { OR: [{ ownerId: uid }, { members: { some: { userId: uid } } }] } } }),
      prisma.task.count({ where: { dueDate: { lt: new Date() }, status: { notIn: ["DONE"] as never[] }, project: { OR: [{ ownerId: uid }, { members: { some: { userId: uid } } }] } } }),
      prisma.user.count(),
    ]);
